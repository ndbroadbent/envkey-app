...
const justInvitedFirstUser = users => {
  ...
  startedOnboardingFn = (props, state) => {
    const users = getColumnsFlattened(props.columnsConfig.columns),
      candidates = R.flatten(
        props.columnsConfig.columns.map(R.prop('candidates'))
      )
    return (
      candidates.length == 0 &&
  ...
  OnboardableAssocManager = Onboardable(AssocManager, AppCollaboratorsSlider, {
    startedOnboardingFn,
...
class AppCollaborators extends React.Component {
  render() {
    ...
    if (this.props.invitingUser || this.props.generatedInviteLink) {
      return h.div([h(InviteUserOverlay, this.props), container])
    } else {
      return container
    }
  }
...
const mapStateToProps = (state, ownProps) => ({
  ...
  mapDispatchToProps = (dispatch, ownProps) => ({
    closeInvite: () =>
      dispatch(closeGeneratedInviteLink({ parentId: ownProps.app.id })),
  })
