  ...
  class SelectedObjectContainer extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
      ...
      if (currentId != nextId) {
        ...
        setTimeout(() => {
          this.setState({ showTransitionOverlay: false })
        }, 1)
    ...
    render() {
      ...
      return h.div(
        '.selected-object-container.show-page',
        {
          className: objectType == 'app' ? `app-role-${obj.role}` : '',
        },
        [
          h.div('.transition-overlay', {
            className: this.state.showTransitionOverlay ? '' : 'hide',
          }),
          h(SelectedTabs, {
  ...
  const mapStateToProps = (state, ownProps) => {
    ...
    mapDispatchToProps = (dispatch, ownProps) => ({
      ...
      selectedObject: object =>
        dispatch(selectedObject({ ...object, objectType })),
    })
