...
const canGenerateServerKeys = props =>
  R.any(R.path(['permissions', 'generateKey']), props.servers)
...
class KeyManager extends React.Component {
  ...
  _renderContents() {
    return [this._renderDevKey(), this._renderServerKeys()]
  }
    ...
    if (canGenerateServerKeys(this.props)) {
      return h.section('.server-keys-section', [
        h.div('.key-section-head', [
          ...
          h.p(
            "Use these keys to give remote servers access to this app's config. Anyone with Devops or Admin privileges can generate and manage these."
          ),
        ]),
...
const mapStateToProps = (state, ownProps) => {
  ...
  startedOnboardingFn = (props, state) => {
    return (
      !(
        props.currentAppUser.role == 'org_owner' && getApps(state).length > 1
      ) &&
  ...
  finishedOnboardingFn = (props, state) =>
    state.startedOnboarding &&
    !state.finishedOnboarding &&
    props.localKeys[0].keyGeneratedAt,
  selectedIndexFn = (props, state) =>
    props.localKeys[0].keyGeneratedAt || state.finishedOnboarding ? 1 : 0,
  OnboardSlider = props => {
  ...
  OnboardableKeyManager = Onboardable(KeyManager, OnboardSlider, {
    startedOnboardingFn,
