...
class Billing extends React.Component {
  ...
  render() {
    return h.div(
      '.billing',
      { className: this._isUpdating() ? 'updating' : '' },
  ...
  _renderFreeTierContents() {
    return [this._renderSubscription(), this._renderUpgrade()]
  }
  ...
  _renderAlert() {
    if (this.props.subscription.status == 'past_due') {
      return h.div('.alert', [
        h.h2('Billing Alert'),
        h.p(
          'Your most recent invoice is past due. Please update your payment details so we can continue to provide service to your organization.'
        ),
        h.p([
  ...
  _renderSubscription() {
    ...
    if (this._isCustomPlan()) {
      contents = [
        h.h3('Custom Plan'),
        <p>
          Your organization is subscribed to a customized plan. Please email{' '}
          <strong>support@envkey.com</strong> to discuss any billing issues.
        </p>,
      ]
      ...
      contents = [
        ...
        h.p(
          `Your ${
            this.props.currentOrg.trialNumDays
          } day free trial will begin after you integrate your first ENVKEY`
        ),
      ]
    } else if (this._isFreeTier()) {
      contents = [
            ...
            [
              [
                ...
                [
                  ...
                  `Up to ${this.props.freePlan.maxKeysPerEnv -
                    1} sub-environment per environment`,
                  `Up to ${
                    this.props.freePlan.maxKeysPerEnv
                  } ENVKEYs per environment`,
                ],
              ],
            ],
          ],
      ...
      contents =
        this.props.currentOrg.pricingVersion >= 4
          ? this._renderBusinessTierPostV4()
          : this._renderBusinessTierPreV4()
    }

    return h.section('.plan', [h.h2('Your Plan'), h.div('.content', contents)])
  }
      ...
      h(BillingColumns, {
        columns: [
          [
            [
              `$${parseInt(
                this.props.subscription.amount / 100
              )}.00 USD / user / month`,
              [
                'Unlimited apps',
          ...
          [
            ['Subscription status', [this._subscriptionStatus()]],
            [
              'Next invoice due',
              [
          ...
          [
            [
              ...
              [
                <span>
                  {this.props.numActiveUsers} {this._renderPendingUsers()}
                </span>,
              ],
            ],

            [
              'Next invoice total charge',
              [
                <span>
                  $
                  {parseInt(
                    (this.props.subscription.amount *
                      this.props.numActiveUsers) /
                      100
                  )}
                  .00 {this._renderPendingCharge()}
                </span>,
              ],
            ],
          ],
  ...
  _renderPendingUsers() {
    ...
    if (numPending > 0) {
      return (
        <small>
          + {numPending} pending invitation{numPending == 1 ? '' : 's'}{' '}
        </small>
      )
    }
  }
    ...
    if (this.props.numPendingUsers > 0) {
      return (
        <small>
          + $
          {parseInt(
            (this.props.subscription.amount * this.props.numPendingUsers) / 100
          )}
          .00 pending
        </small>
      )
  ...
  _renderUpgrade() {
    ...
    return h.section('.upgrade', [h.h2('Upgrade'), h.div('.content', contents)])
  }
      ...
      h(BillingColumns, {
        columns: [
          [
            [
              `$${parseInt(
                this.props.businessPlan.amount / 100
              )}.00 / user / month`,
              ['Unlimited apps', 'Unlimited environments', 'Unlimited ENVKEYs'],
            ],
          ],
        ],
      }),
      h.div('.actions', [this._renderUpgradeButton()]),
    ]
  }
  ...
  _renderUpgradeButton() {
    if (this.state.willShowUpgradeForm) {
      ...
      return h.button(
        ...
        `Upgrade To ${this.props.businessPlan.name}`
      )
    }
  }
  ...
  _renderCancel() {
    if (this.state.confirmCancel) {
      ...
      this.props.currentOrg.pricingVersion == 1 &&
      this.props.isExceedingFreeTier
    ) {
      return (
        <div className="actions">
          <Link
            to={`/${this.props.currentOrg.slug}/downgrade_removal`}
            className="button"
          >
  ...
  _renderInvoiceSettings() {
    return (
      <section className="billing-invoice-settings">
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    updateInvoiceSettings: (targetId, params) =>
      dispatch(
        updateObjectSettings({
          objectType: 'org',
          targetId,
