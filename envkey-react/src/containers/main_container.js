...
const appStateLoaded = props => {
  return !props.isLoadingAppState && props.currentUser && props.currentOrg
}
...
class Main extends React.Component {
  ...
  _classNames() {
    return [
      ...
      this._shouldShowTrialAlert() || this.props.isDemo
        ? 'show-bottom-alert'
        : '',
    ]
  }
  ...
  render() {
    if (appStateLoaded(this.props)) {
      return (
        <div className={this._classNames().join(' ')}>
  ...
  _renderContents() {
    if (
      this._trialOverdue() &&
      !this.props.location.pathname.endsWith('/downgrade_removal')
    ) {
  ...
  _renderDemoCTA() {
    if (this.props.isDemo) {
      return (
        <div className="bottom-alert demo-cta">
          <div>
            <span>
              <strong>This is a live demo.</strong> You shouldn't enter any real
              secrets. To start a free trial, download the EnvKey App.
            </span>
          </div>

          <a
            className="button"
            href={this.props.demoDownloadUrl}
            target="_blank"
          >
            Download{' '}
          </a>
  ...
  _renderTrialAlert() {
    if (this._shouldShowTrialAlert()) {
      if (this.state.willShowUpgradeForm || this.props.isUpdatingSubscription) {
        return (
          <div className="bottom-alert trial-alert">
            <Spinner />
          </div>
        )
      }
  ...
  _renderTrialAlertCopy() {
    if (this.props.currentOrg.trialDaysRemaining > 28) {
      return (
        <span>
          {' '}
          An ENVKEY was integrated, so {this.props.currentOrg.name}'s{' '}
          <strong>Free Trial</strong> has started.{' '}
          <strong>{this.props.currentOrg.trialDaysRemaining} days</strong> are
          left.{' '}
        </span>
      )
    } else {
      return (
        <span>
          {' '}
          {this.props.currentOrg.name}'s <strong>Free Trial</strong> has{' '}
          <strong>{this.props.currentOrg.trialDaysRemaining} days</strong> left.{' '}
        </span>
      )
    }
  }
