...
const SettingsFormContainerFactory = ({
  objectType,
  targetObjectType,
  targetObjectPath,
}) => {
  ...
  const SettingsFormContainer = props =>
      h.div('.settings', [h(formClass, props)]),
    ...
    mapDispatchToProps = (dispatch, ownProps) => {
      const obj = ownProps[objectType],
        { id: targetId } = targetObjectPath
          ? R.path(targetObjectPath, obj)
          : obj,
      ...
      return {
        ...
        onUpdateSettings: params =>
          dispatch(updateObjectSettings({ ...baseParams, params })),
        onUpdateOrgRole: ({ role }) =>
          dispatch(
            updateOrgRole({ role, userId: obj.id, orgUserId: targetId })
          ),
      }
    }
