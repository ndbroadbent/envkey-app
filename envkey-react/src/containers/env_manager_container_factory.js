...
const EnvManagerContainerFactory = ({ parentType }) => {
  const mapStateToProps = (state, ownProps) => {
      ...
      const currentUser = getCurrentUser(state),
        ...
        envsWithMetaWithPending = getEnvsWithMetaWithPending(
          parentType,
          appId,
          state
        )
      ...
      return {
        ...
        isUpdatingValFn: (entryKey, environment) =>
          getIsUpdatingEnvVal({ appId, entryKey, environment }, state),
        isUpdatingEntryFn: entryKey =>
          getIsUpdatingEnvEntry({ appId, entryKey }, state),
        ...
        environmentsAssignable: getCurrentUserEnvironmentsAssignableForApp(
          appId,
          state
        ),
    ...
    mapDispatchToProps = (dispatch, ownProps) => {
      ...
      return {
        dispatch,
        createEntry: ({ entryKey, vals, subEnvId }) =>
          dispatch(
            createEntry({
              ...baseProps,
              entryKey,
        ...
        updateEntry: (entryKey, newKey, subEnvId) =>
          dispatch(
            updateEntry({
              ...baseProps,
              entryKey,
        ...
        removeEntry: (entryKey, subEnvId) =>
          dispatch(removeEntry({ ...baseProps, entryKey, subEnvId })),
        updateEntryVal: (entryKey, environment, update, subEnvId) =>
          dispatch(
            updateEntryVal({
              ...baseProps,
              entryKey,
        ...
        removeSubEnv: params =>
          dispatch(removeSubEnv({ ...baseProps, ...params })),
        renameSubEnv: params =>
          dispatch(renameSubEnv({ ...baseProps, ...params })),

        editCell: (entryKey, environment, subEnvId) => {
          if (entryKey && environment) {
            dispatch(
              socketUpdateLocalStatus({
                editingEntryVal: { [entryKey]: { [environment]: true } },
              })
            )
          } else if (entryKey) {
            dispatch(
              socketUpdateLocalStatus({
                editingEntry: {
                  [entryKey]: { [subEnvId || '@@__base__']: true },
                },
              })
            )
          }
        ...
        addingEntry: subEnvId =>
          dispatch(
            socketUpdateLocalStatus({ addingEntry: subEnvId || '@@__base__' })
          ),
    ...
    startedOnboardingFn = (props, state) => {
      ...
      return (
        ((props.parent.role == 'org_owner' &&
          allEntries(props.envsWithMeta).length == 0) ||
          (props.parent.role != 'org_owner' &&
            props.isInvitee &&
    ...
    OnboardableEnvManager = Onboardable(EnvManager, OnboardSlider, {
      startedOnboardingFn,
