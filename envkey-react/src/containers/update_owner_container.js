...
class UpdateOwner extends React.Component {
  ...
  render() {
    return (
      <form className="update-owner object-form">
        <fieldset>
  ...
  _renderNoAdmins() {
    return (
      <p>
        Only an Org Admin can be made an Org Owner. {this.props.currentOrg.name}{' '}
        has no active Org Admins.
      </p>
    )
  }
  ...
  _renderAdminOpts() {
    return [
      <option key={0} disabled={true} selected={true} value="">
        Choose an Org Admin
      </option>,
    ].concat(
      this.props.orgAdmins.map(({ id, firstName, lastName }, i) => {
        return (
          <option key={i + 1} value={id}>
            {firstName} {lastName}
          </option>
        )
      })
      ...
      return (
        <fieldset>
          <button
            ...
            disabled={this.state.newOwnerId == null}
          >
            Change Owner
          </button>
        </fieldset>
