...
import { accountResetOptionsRequest, removeObject } from 'actions'
import {
      ...
      <OnboardOverlay>
        <div>
          ...
          <p className="small">
            ...
            Next, ask an admin of each org you're a member of to re-invite you.{' '}
            <br />
    ...
    if (this.props.accountResetOptions) {
      return (
        <div className="org-sections">
          {R.toPairs(this.props.accountResetOptions).map(
            ::this._renderOrgSection
          )}
        </div>
      )
    } else {
    ...
    if (users.length) {
      return (
        <div className="org-users">
          {users.map(userStr => (
            <div className="org-user">{userStr}</div>
          ))}
        </div>
      )
    } else {
      return (
        <div className="org-users no-users">
