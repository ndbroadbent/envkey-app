...
function* onVerifyOrgPubkeys() {
    ...
    for (let keyable of keyables) {
      ...
      if (trusted && keyableTrusted) {
        ...
        if (doLogging)
          console.log(
            'Keyable ',
            keyableId,
            'has no initialSignedById or initialInvitedById--marking unverified...'
          )
      ...
      while (!trustedRoot) {
        ...
        if (unverifiedKeyables[signingId]) {
          if (doLogging)
            console.log('signing user marked unverified... breaing')
          break
        }
        ...
        if (signingUser && signingUser.pubkey) {
          let signedKey = invitedById
              ? checkingKeyable.invitePubkey
              : checkingKeyable.pubkey,
            verified

          if (doLogging) console.log('Verifying signed key.')
          try {
            verified = crypto.verifyPublicKeySignature({
              signedKey,
              signerKey: signingUser.pubkey,
            })
        ...
        if (checkingKeyable.invitePubkey && checkingKeyable.pubkey) {
          ...
          try {
            verified = crypto.verifyPublicKeySignature({
              signedKey: checkingKeyable.pubkey,
              signerKey: checkingKeyable.invitePubkey,
            })
          } catch (e) {
            console.log('Verification error: ', e)
          }
        ...
        if (newlyTrustedKeyables[signingId]) {
          ...
          if (doLogging)
            console.log(
              'Signing id not yet verified... looking in trustedPubkeys'
            )
          trustedRoot = yield call(keyableIsTrusted, signingUser)
            ? trustedPubkeys[signingId]
            : null
        }
  ...
  if (R.isEmpty(unverifiedKeyables)) {
    ...
    yield put({
      type: VERIFY_ORG_PUBKEYS_SUCCESS,
      payload: R.values(newlyTrustedKeyables),
    })
  } else {
    ...
    yield put({
      type: VERIFY_ORG_PUBKEYS_FAILED,
      error: true,
      payload: R.values(unverifiedKeyables),
    })
  }
}
...
function* onUpdateEncryptedPrivkey({ payload: { oldPassword, newPassword } }) {
  ...
  if (res.type == DECRYPT_PRIVKEY_FAILED) {
    yield put({
      type: UPDATE_ENCRYPTED_PRIVKEY_FAILED,
      error: true,
      payload: 'Invalid passphrase',
    })
  } else {
    const privkey = yield select(getPrivkey),
      encryptedPrivkey = crypto.encryptPrivateKey({
        privkey,
        passphrase: newPassword,
      })

    yield put({
      type: UPDATE_ENCRYPTED_PRIVKEY_REQUEST,
      payload: { encryptedPrivkey },
    })
  }
}
...
function* onVerifyTrustedPubkeys() {
  ...
  if (trustedPubkeys && !R.isEmpty(trustedPubkeys)) {
    ...
    yield put({
      type: VERIFY_TRUSTED_PUBKEYS_FAILED,
      error: true,
      payload: 'No trusted pubkeys',
    })
  } else {
    try {
      ...
      yield put({
        type: VERIFY_TRUSTED_PUBKEYS_SUCCESS,
        payload: trustedPubkeys,
      })
    } catch (err) {
      yield put({
        type: VERIFY_TRUSTED_PUBKEYS_FAILED,
...
function* onGenerateUserKeypair({ payload: { email, password } }) {
  ...
  yield put({
    type: GENERATE_USER_KEYPAIR_SUCCESS,
    payload: { encryptedPrivkey, pubkey },
  })
}
...
function* onDecryptEnvs(action) {
  const {
      meta: { objectType, targetId, decryptAllAction, background },
    } = action,
  ...
  if (!(!background && decryptAllAction && envsAreDecrypted)) {
    try {
      ...
      yield put({
        type: DECRYPT_ENVS_SUCCESS,
        payload: decryptedParent,
        meta: action.meta,
      })
    } catch (err) {
      yield put({
        type: DECRYPT_ENVS_FAILED,
        error: true,
        payload: err,
        meta: action.meta,
      })
...
export default function* cryptoSagas() {
  yield [
    ...
    takeLatest(
      UPDATE_ENCRYPTED_PRIVKEY_REQUEST,
      onUpdateEncryptedPrivkeyRequest
    ),
