...
const onBillingUpdateSubscriptionRequest = apiSaga({
    authenticated: true,
    method: 'patch',
    actionTypes: [
      BILLING_UPDATE_SUBSCRIPTION_SUCCESS,
    ...
    urlFn: (action, currentOrg) =>
      `/orgs/${currentOrg.id}/update_subscription.json`,
  }),
  onBillingUpdateCardRequest = apiSaga({
    ...
    urlSelector: getCurrentOrg,
    urlFn: (action, currentOrg) =>
      `/orgs/${currentOrg.id}/update_stripe_card.json`,
  }),
  ...
  while (true) {
    ...
    if (formResult.type == BILLING_STRIPE_FORM_SUBMITTED) {
      yield put(
        billingUpdateSubscriptionRequest({
          ...formResult.payload,
          planId: currentOrg.businessPlan.id,
          updateType: 'upgrade',
        })
      )
    }
...
function* onBillingCancelSubscription({ payload }) {
  ...
  if (currentOrg.pricingVersion == 1) {
    ...
    planId = currentOrg.trialStartedAt
      ? currentOrg.trialPlan.id
      : currentOrg.preTrialPlan.id
  }
...
function saveFile(invoice) {
  return new Promise(resolve => {
    window.dialog.showSaveDialog(
      {
        title: 'Save Invoice',
        defaultPath: `envkey-invoice-${moment(invoice.createdAt).format(
          'YYYY-MM-DD'
        )}.pdf`,
      },
      resolve
...
function writeFile(filename, data) {
  ...
  return new Promise(resolve => {
    fileReader.onload = function() {
      fs.writeFile(filename, Buffer.from(new Uint8Array(this.result)), resolve)
    }
...
export default function* billingSagas() {
  yield [
    ...
    takeLatest(
      BILLING_UPDATE_SUBSCRIPTION_REQUEST,
      onBillingUpdateSubscriptionRequest
    ),
    takeLatest(BILLING_UPDATE_CARD_REQUEST, onBillingUpdateCardRequest),
    takeLatest(
      BILLING_FETCH_INVOICE_LIST_REQUEST,
      onBillingFetchInvoiceListRequest
    ),
