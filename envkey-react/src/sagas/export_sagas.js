...
import { getApp, getRawEnvWithPendingForApp } from 'selectors'
import { EXPORT_ENVIRONMENT } from 'actions'
...
function* onExportEnvironment({
  meta: { parentId },
  payload: { environment, format, subEnvId, subEnvName },
}) {
  const app = yield select(getApp(parentId)),
    rawEnv = yield select(
      getRawEnvWithPendingForApp({ appId: parentId, environment, subEnvId })
    )
...
export default function* importSagas() {
  yield [takeEvery(EXPORT_ENVIRONMENT, onExportEnvironment)]
}
