...
export function* keyableIsTrusted(keyable) {
  ...
  if (!(pubkey || invitePubkey)) {
    if (doLogging)
      console.log(
        'keyableIsTrusted: Missing either pubkey or invitePubkey. Not trusted.'
      )
    return false
  }
  ...
  if (
    (pubkey && !trusted.pubkeyFingerprint) ||
    (invitePubkey && !trusted.invitePubkeyFingerprint)
  ) {
    if (doLogging)
      console.log(
        'keyableIsTrusted: Trusted keyable is missing either pubkeyFingerprint or invitePubkeyFingerprint.'
      )
    return false
  }
  ...
  if (!R.equals(keyableProps, trustedProps)) {
    if (doLogging)
      console.log(
        'keyableIsTrusted: keyable props do not match trusted keyable props.'
      )
    return false
  }
  ...
  if (
    pubkey &&
    crypto.getPubkeyFingerprint(pubkey) != trusted.pubkeyFingerprint
  ) {
    if (doLogging)
  ...
  if (
    invitePubkey &&
    crypto.getPubkeyFingerprint(invitePubkey) != trusted.invitePubkeyFingerprint
  ) {
    if (doLogging)
      console.log(
        'keyableIsTrusted: invitePubkeyFingerprint does not match invitePubkey'
      )
    return false
  }
  ...
  if (parent.encryptedEnvsWithMeta) {
    ...
    for (let environment in parent.encryptedEnvsWithMeta) {
      ...
      if (!encrypted || R.isEmpty(encrypted)) {
        ...
        if (!signedById)
          throw new Error(
            `Parent ${parent.id} ${environment} environment not signed.`
          )
        ...
        decrypted[environment] = yield crypto.decryptJson({
          encrypted,
          privkey,
