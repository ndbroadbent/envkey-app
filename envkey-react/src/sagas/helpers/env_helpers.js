...
export function* dispatchEnvUpdateRequest(params) {
  const { meta = {}, parentId, parentType, skipDelay, updatePending } = params,
    envUpdateId =
      meta.forceEnvUpdateId || (yield select(getEnvUpdateId(parentId)))
  ...
  const parent = yield select(getObject(parentType, parentId)),
    ...
    envActionsPendingBefore = yield select(
      getEnvActionsPendingByEnvUpdateId(parentId, envUpdateId)
    ),
    envParams = yield call(envParamsForApp, { appId: parentId, envsWithMeta }),
    signedByTrustedPubkeys = yield call(signTrustedPubkeyChain),
    envActionsPending = yield select(
      getEnvActionsPendingByEnvUpdateId(parentId, envUpdateId)
    )
...
export function* clearSubEnvServersIfNeeded({
  meta: { envActionsPending, parentId },
}) {
  const removeSubEnvActions = envActionsPending.filter(
    R.propEq('type', REMOVE_SUB_ENV)
  )

  for (let {
    meta: { id },
  } of removeSubEnvActions) {
...
export function* addDefaultSubEnvServerIfNeeded({
  meta: { envActionsPending, parentId },
}) {
  const addSubEnvActions = envActionsPending.filter(
    R.propEq('type', ADD_SUB_ENV)
  )

  for (let {
    payload: { environment, name, id: subEnvId },
  } of addSubEnvActions) {
    yield put(
      addAssoc({
        parentId,
