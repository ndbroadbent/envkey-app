...
import { TOKEN_INVALID, ORG_INVALID, API_SUCCESS, API_FAILED } from 'actions'
...
export default function apiSaga({
  ...
  if (
    ...
    throw new Error(
      'actionTypes: Expected an array of two string actionTypes, for success and failure conditions.'
    )
  }

  if (['get', 'post', 'patch', 'put', 'delete'].indexOf(method) == -1) {
    throw new Error(
      'method: Requires a valid REST method: get, post, patch, put, or delete.'
    )
  }
  ...
  if (
    !(typeof urlSelector === 'function' || typeof urlSelector === 'undefined')
  ) {
    throw new Error(
      'urlSelector: Expected to be either undefined or a function.'
    )
  }
  ...
  return function*(requestAction) {
      ...
      const hasPasswordKey = R.pipe(
        ...
        R.any(
          k =>
            ...
            k.toLowerCase().indexOf('passphrase') > -1
        )
    ...
    const auth = yield select(getAuth),
      orgSlug =
        R.path(['meta', 'orgSlug'], requestAction) ||
        (yield select(getCurrentOrgSlug)),
      ...
      params =
        method == 'get' ? { ...orgParams, ...decamelizedPayload } : orgParams,
      data = method == 'get' ? {} : decamelizedPayload,
      config = { method, url, params, data, responseType }

    try {
      const [res] =
        minDelay && !R.path(['meta', 'skipDelay'], requestAction)
