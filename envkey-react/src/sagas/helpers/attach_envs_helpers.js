...
export function* envParamsWithAppUser(
  { appId, userId, role, withTrustedPubkey, isAcceptingInvite = false },
  envParams = {}
) {
  const privkey = yield select(getPrivkey),
    ...
    environments = yield select(
      getCurrentUserEnvironmentsAssignableToAppUser({ appId, userId, role })
    ),
  ...
  if (pubkey && (withTrustedPubkey || (yield call(keyableIsTrusted, user)))) {
    const app = yield select(getApp(appId)),
      ...
      encryptedEnvs = yield environments.map(environment =>
        encryptJson({ data: envsWithMeta[environment], pubkey, privkey })
      )
...
export function* envParamsWithServer({ appId, serverId }, envParams = {}) {
  const privkey = yield select(getPrivkey),
    ...
    environment = yield select(
      getCurrentUserEnvironmentAssignableToServer({ appId, serverId })
    )

  if (!(yield call(keyableIsTrusted, server))) return envParams

  if (environment) {
    const rawEnv = yield select(
      getRawEnvWithPendingForApp({ appId, environment, subEnvId })
    )
    return R.assocPath(
      ['servers', serverId],
      {
    ...
    if (appRole == 'development' && serverRole == 'production') {
      const envsWithMeta = yield select(
          getEnvsWithMetaWithPending('app', appId)
        ),
      ...
      if (R.isEmpty(inheritanceOverrides)) {
        ...
        return R.assocPath(
          ['servers', serverId],
          {
            inheritanceOverrides: yield encryptJson({
              data: inheritanceOverrides,
              pubkey,
              privkey,
            }),
          },
...
export function* envParamsWithLocalKey({ appId, localKeyId }, envParams = {}) {
  ...
  const rawEnv = yield select(
    getRawEnvWithPendingForApp({ appId, environment })
  )
  return R.assocPath(
    ['localKeys', localKeyId],
    {
      env: yield encryptJson({ data: rawEnv, pubkey, privkey }),
    },
    envParams
...
export function* envParamsForApp({ appId }) {
  const users = yield select(getUsersForApp(appId)),
    ...
    userGenerators = users.map(({ id: userId }) =>
      call(envParamsWithAppUser, { appId, userId })
    ),
    serverGenerators = servers.map(({ id: serverId }) =>
      call(envParamsWithServer, { appId, serverId })
    ),
    localKeyGenerators = localKeys.map(({ id: localKeyId }) =>
      call(envParamsWithLocalKey, { appId, localKeyId })
    ),
...
export function* envParamsForInvitee({ userId, permittedAppIds }) {
  const generators = permittedAppIds.map(appId =>
      call(envParamsWithAppUser, { userId, appId })
    ),
...
export function* envParamsForAcceptedInvite(withTrustedPubkey) {
  const { id: userId } = yield select(getCurrentUser),
    ...
    generators = apps.map(({ id: appId }) =>
      call(envParamsWithAppUser, {
        userId,
        appId,
        withTrustedPubkey,
...
export function* envParamsForUpdateOrgRole({ userId, role: newRole }) {
  ...
  const apps = yield select(getApps),
    generators = apps.map(({ id: appId }) =>
      call(envParamsWithAppUser, { userId, appId, role: newRole })
    ),
    allParams = yield generators,
    merged = allParams.reduce(R.mergeDeepRight)
