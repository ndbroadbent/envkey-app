...
const onFetchCurrentUserRequest = apiSaga({
    authenticated: true,
    method: 'get',
...
function* onLoginSuccess({ meta: { password, orgSlug } }) {
  if (orgSlug) {
    ...
    yield orgs.length == 1 && orgs[0].isActive
...
function* onRegister({ payload }) {
  ...
  const {
    payload: { pubkey, encryptedPrivkey },
  } = yield take(GENERATE_USER_KEYPAIR_SUCCESS)
...
function* onRegisterSuccess({
  ...
  const currentOrg = yield select(getCurrentOrg),
    ...
    [, decryptPrivkeyResult] = yield [
      put(
        addTrustedPubkey({
          keyable: { type: 'user', ...currentUser, pubkey },
          orgId: currentOrg.id,
        })
      ),
  ...
  if (!decryptPrivkeyResult.error) {
    const updateTrustedRes = yield call(
      execUpdateTrustedPubkeys,
      currentOrg.slug
    )
...
function* onApiFailed({ payload, meta }) {
  if (
    R.path(['response', 'status'], payload) == 403 &&
    meta.message == 'Unauthorized IP'
  ) {
    alert(
      `This Org does not allow EnvKey App requests from your current IP (${
...
export default function* authSagas() {
  yield [
    ...
    takeLatest(
      FETCH_CURRENT_USER_UPDATES_REQUEST,
      onFetchCurrentUserUpdatesRequest
    ),
    ...
    takeLatest(
      FETCH_CURRENT_USER_UPDATES_API_SUCCESS,
      onFetchCurrentUserUpdatesApiSuccess
    ),
    ...
    takeLatest(
      [SELECT_ACCOUNT_FAILED, LOGIN_FAILED, REGISTER_FAILED],
      onAuthFailed
    ),
