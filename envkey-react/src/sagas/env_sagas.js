...
import { getEnvActionsPendingByEnvUpdateId, getObject } from 'selectors'
import {
  UPDATE_ENV_REQUEST,
  UPDATE_ENV_SUCCESS,
  UPDATE_ENV_FAILED,
  ...
  if (isOutdatedEnvsResponse(action)) {
    ...
    for (let pendingAction of envActionsPending) {
      try {
        ...
        yield put({
          ...pendingAction,
          meta: { ...pendingAction.meta, parent: reloadedParent },
        })
      } catch (e) {
        console.log('Replay pending envs error')
        console.log(e)
        alert(
          `There was a problem applying your update to ${
            pendingAction.payload.entryKey
          }. This variable may have been deleted or renamed by another user.`
        )
      }
    }
