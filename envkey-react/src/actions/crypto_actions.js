...
export const decryptAll = createAction(
    DECRYPT_ALL,
    R.pick(['password']),
    R.pick(['background'])
  ),
  decryptPrivkey = createAction(DECRYPT_PRIVKEY, R.pick(['password'])),
  decryptEnvs = createAction(
    DECRYPT_ENVS,
    R.always({}),
    R.pick(['objectType', 'targetId', 'decryptAllAction', 'background'])
  ),
  updateTrustedPubkeys = createAction(
    UPDATE_TRUSTED_PUBKEYS,
    R.always({}),
    R.pick(['orgSlug'])
  ),
  ...
  verifyCurrentUserPubkey = createAction(
    VERIFY_CURRENT_USER_PUBKEY,
    R.always({}),
    R.pick(['background'])
  ),
