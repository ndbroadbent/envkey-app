...
const defaultAccountMenuExpanded = props =>
  props.location.pathname.includes('/my_org/') ||
  props.location.pathname.includes('/my_account/')
...
export default class Sidebar extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      !R.equals(this.props.params, nextProps.params)
    ) {
  ...
  _renderFilter() {
    if (this._showFilter()) {
      return (
        <div
          className={
            'filter-row ' + (this.state.accountMenuOpen ? ' hide' : '')
          }
        >
          <Filter
      ...
      >
        ...
        {this._renderMenuListSection(
          ...
          'menu-user-white.svg',
          {
            ...
            pathFn: item =>
              `/${this.props.currentOrg.slug}/users/${item.slug}/settings`,
            labelFn: item => [
              <span key="0">{item.firstName + ' '}</span>,
