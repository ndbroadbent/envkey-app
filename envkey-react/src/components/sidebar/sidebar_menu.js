...
export default class SidebarMenu extends React.Component {
  ...
  render() {
    ...
    return (
      <div
        className={
          'side-menu ' + this.props.type + (this.state.open ? ' open' : '')
        }
      >
  ...
  _renderMenuLabel(filtered, numItems) {
    if (numItems > 0) {
      return (
        <div className="menu-label" onClick={::this._onClickLabel}>
        ...
        return (
          <div key={i} className="menu-group">
            <div className="group-label">
              {this.props.groupLabelFn ? this.props.groupLabelFn(group) : group}
            </div>
    ...
    return items.map((item, i) => {
      ...
      if (
        this.props.location.pathname.includes(
          `/${this.props.type}/${this.props.params.slug}`
        )
      ) {
        // Don't redirect into subenv for apps (should clean this up)
        if (
          this.props.location.pathname.includes(
            `/${this.props.params.slug}/variables/`
          )
        ) {
  ...
  _renderNewButton() {
    if (
      ...
      const newSelected = this.props.location.pathname.endsWith(
        `/${this.props.type}/new`
      )
      return (
        <Link
          to={`/${this.props.params.orgSlug}/${this.props.type}/new`}
          className={
            'button img-button new-btn' + (newSelected ? ' selected' : '')
          }
        >
          <img
            src={imagePath(
              newSelected ? 'circle-plus-white.svg' : 'circle-plus-orange.svg'
            )}
          />
