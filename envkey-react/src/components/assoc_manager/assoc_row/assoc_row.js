...
const targetId = props =>
    R.path(['relation', 'id'], props.assoc) || props.assoc.id,
  canRemove = ({ assoc }) =>
    !assoc.isDefault &&
    (assoc.relation
      ? Boolean(assoc.relation.permissions.delete)
      : Boolean(assoc.permissions.delete)),
  isRemoving = props => props.isRemovingById[targetId(props)] || false,
  isGeneratingAssocKey = props =>
    props.isGeneratingAssocKeyById[targetId(props)] || false,
  isRevokingAssocKey = props =>
    props.isRevokingAssocKeyById[targetId(props)] || false,
  isGrantingEnvAccess = ({
    isGrantingEnvAccessByUserId,
    parent: { id: parentId },
    assoc: { id: assocId },
  }) =>
    isGrantingEnvAccessByUserId[assocId] ||
    isGrantingEnvAccessByUserId[parentId] ||
    false

class AssocRow extends React.Component {
  _onRemove() {
    this.props.removeAssoc({
      targetId: targetId(this.props),
      assocId: this.props.assoc.id,
    })
  }
