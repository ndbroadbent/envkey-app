...
export default class AssocColumn extends React.Component {
  ...
  _canAdd() {
    const canAdd =
      (this.props.parentType == 'user' ||
        R.path(
          [this.props.joinType, 'create'],
          this.props.parent.permissions
        )) &&
  ...
  _parentName() {
    ...
    return parentNameFn
      ? parentNameFn(this.props.parent)
  ...
  _addLabel() {
    ...
    if (!labelOrFn) return '+'
    return typeof labelOrFn == 'function'
      ? labelOrFn(this.props.config)
      : labelOrFn
  }
  ...
  _renderAddButton() {
    ...
    if (this.props.config.isAddingAssoc || this.props.config.isCreating) {
      ...
      return h.button(
        ...
        {
          onClick: e =>
            this.setState((state, props) => ({ addMode: !state.addMode })),
        },
  ...
  _renderPermissions() {
    ...
    if (permissionCopyLines && permissionCopyLines.length) {
      return h.div('.permissions', [
        h.ul(
          permissionCopyLines.map((permission, i) =>
            h.li({ key: i }, permission)
          )
        ),
      ])
    }
  ...
  _renderHeader() {
    ...
    return h.div('.column-header', [
      h.div('.head', [
        ...
        h.h4('.title', [h.em(title), subtitle ? [' ', subtitle] : '']),
  ...
  _renderSections() {
    const sections = R.values(
      R.mapObjIndexed(::this._renderSection, this.props.config.groups)
    )
    if (sections.length) {
  ...
  _renderSection(associations, k) {
    if (associations.length) {
      const showSectionFn =
        this.props.config.showSectionFn ||
        this.props.columnsConfig.showSectionFn
  ...
  _renderSectionTitle(k) {
    const sectionTitleFn =
        this.props.config.sectionTitleFn ||
        this.props.columnsConfig.sectionTitleFn,
      sectionSubtitleFn =
        this.props.config.sectionSubtitleFn ||
        this.props.columnsConfig.sectionSubtitleFn
    if (sectionTitleFn) {
      return h.div('.section-title', [
        h.span('.title', sectionTitleFn(k, this.props)),
        h.span(
          '.subtitle',
          sectionSubtitleFn ? sectionSubtitleFn(k, this.props) : ''
        ),
      ])
    }
  }
