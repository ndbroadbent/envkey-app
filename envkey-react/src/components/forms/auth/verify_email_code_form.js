...
export default class VerifyEmailCodeForm extends React.Component {
      ...
      h.form({ onSubmit: this.props.onSubmit }, [
        h.fieldset([
          h.input({
            type: 'password',
            disabled:
              this.props.isVerifyingEmailCode || this.props.isAuthenticating,
        ...
        h.fieldset([this._renderVerifyEmailCodeSubmit()]),
      ]),
    ])
  }
  ...
  _renderVerifyEmailCodeSubmit() {
    if (this.props.isVerifyingEmailCode || this.props.isAuthenticating) {
      ...
      const label =
        this.props.emailVerificationType == 'sign_in' ? 'Sign In' : 'Next'
  ...
  _renderVerifyEmailCodeError() {
    ...
    if (this.props.verifyEmailCodeError || this.props.authError) {
      return h.p('.error', [
        `Oops! We couldn't verify your email. Check your internet connection, confirm you have the right ${codeName}, and try again. If it's still not working, contact support: support@envkey.com`,
      ])
    }
  }
}
