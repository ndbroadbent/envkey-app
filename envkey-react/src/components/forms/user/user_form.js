...
export default class UserForm extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      R.path(['user', 'id'], this.props) != R.path(['user', 'id'], nextProps)
    ) {
  ...
  _showSubscriptionWall() {
    return (
      ...
      this.props.numUsers >= this.props.currentOrg.maxUsers
    )
  }
  ...
  render() {
    ...
    return h.form(
      '.object-form.new-form.invite-new-user',
      {
        ref: 'form',
  ...
  _renderOrgAdminToggle() {
    if (
      this.props.addAssoc &&
      this.props.orgRolesInvitable &&
      this.props.orgRolesInvitable.includes('org_admin')
    ) {
      return h.label(
        '.org-admin-toggle',
        {
        ...
        [
          h.span(
            this.state.firstName
              ? ['Make ', h.em(this.state.firstName), ' an org admin']
              : 'Make org admin'
          ),
          h.input({
            type: 'checkbox',
            ...
            onChange: () =>
              this.setState({
                orgRole:
                  this.state.orgRole == 'org_admin' ? 'basic' : 'org_admin',
              }),
          }),
  ...
  _renderOrgRoleSelect() {
    if (
      !this.props.addAssoc &&
      this.props.orgRolesAssignable &&
      this.props.orgRolesAssignable.length
    ) {
