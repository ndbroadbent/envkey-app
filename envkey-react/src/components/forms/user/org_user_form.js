...
const defaultState = props => R.pick(['role'], props.orgUser)
...
export default class OrgUserForm extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      R.path(['orgUser', 'id'], this.props) !=
      R.path(['orgUser', 'id'], nextProps)
    ) {
  ...
  render() {
    return h.form(
      '.object-form.org-user',
      {
        ref: 'form',
        onSubmit: ::this._onSubmit,
      },
      [this._renderOrgRoleSelect(), h.fieldset([this._renderSubmit()])]
    )
  }
