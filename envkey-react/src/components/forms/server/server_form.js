...
export default class ServerForm extends React.Component {
  ...
  _onSubmit(e) {
    ...
    this.props.onSubmit({
      ...
      subEnvId: this.state.selectedSubEnv,
    })
  }
  ...
  render() {
    if (this._showSubscriptionWall()) {
      return (
        <SubscriptionWallContainer
          subject={`The ${this.props.role} environment`}
          type="key"
    ...
    return (
      <form
        className="object-form add-server"
  ...
  _renderSubEnvSelect() {
    if (this.props.subEnvOpts.length) {
      return (
        <fieldset className="full-select">
  ...
  _renderSubEnvOptions() {
    return [
      ...
      this.props.subEnvOpts.map(({ id, name }, i) => {
        return (
