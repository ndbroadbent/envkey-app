...
export default class AppForm extends React.Component {
  ...
  _onSubmit(e) {
    ...
    this.props.onSubmit({
      ...
      toImport: this._willImport()
        ? this.refs.appImporter.toImport()
        : undefined,
  ...
  _showSubscriptionWall() {
    return (
      ...
      this.props.numApps >= this.props.currentOrg.maxApps
    )
  }
  ...
  _renderImportOpts() {
    return (
      <fieldset className="radio-opts import-opts">
        <RadioGroup
          selectedValue={this.state.importOption}
          onChange={::this._onImportOptionChange}
        >
          <label
            className={this.state.importOption == 'noImport' ? 'selected' : ''}
          >
            <Radio disabled={this.props.isSubmitting} value="noImport" />{' '}
            <strong>Start from scratch</strong>
          </label>
          <label
            className={this.state.importOption == 'import' ? 'selected' : ''}
          >
            <Radio disabled={this.props.isSubmitting} value="import" />
  ...
  _renderSubmit() {
    if (this.props.isSubmitting) {
      ...
      if (!this.props.renderImporter && this.state.importOption == 'import') {
        ...
        const disabled =
          this.state.importOption == 'import' && !this.state.importValid
