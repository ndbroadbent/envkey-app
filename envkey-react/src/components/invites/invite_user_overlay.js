...
export default class InviteUserOverlay extends React.Component {
  ...
  _onCopy() {
    const { identityHash, passphrase } = this.props.generatedInviteLink,
      ...
      res = copy(encryptionToken, {
        message: 'Copy the text below with #{key}',
      })
    if (res) {
  ...
  _renderInvitedCopy(firstName) {
    if (this.props.isDemo) {
      return h.div('.copy', [
        h.p([
          h.strong("If this weren't a demo, "),
          `an EnvKey invitation would be sent to ${firstName} by email.`,
        ]),
        h.p([
  ...
  _renderGeneratedInviteLink() {
    const {
      ...
      } = this.props.generatedInviteLink,
      encryptionToken = [identityHash, passphrase].join('_')
