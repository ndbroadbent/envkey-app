...
export default class DecryptForm extends React.Component {
  ...
  render() {
    return h.div('.viewport-overlay', [
      h.form('.auth-form.decrypt-form', { onSubmit: ::this._onSubmit }, [
        ...
        h.p('.msg.msg-reset-account', [
          'Forgot your passphrase? ',
          h(Link, { to: '/reset_account' }, ['Reset your account.']),
        ]),
      ]),
    ])
  }
  ...
  _renderError() {
    if (this.props.decryptPrivkeyErr) {
      return h.div('.msg', [h.p('Incorrect passphrase. Please try again.')])
    } else if (this.props.decryptAllErr) {
      return h.div(
        '.msg',
        'There was a problem decrypting and verifying your config. Enter your passphrase and try again. If the problem persists, please email support@envkey.com.'
      )
    }
  }
}
