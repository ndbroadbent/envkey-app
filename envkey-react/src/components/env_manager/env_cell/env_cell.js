...
export default class EnvCell extends React.Component {
  ...
  _classNames() {
    return ['cell', this.props.isUpdating ? 'updating' : '']
  }
  ...
  render() {
    return h.div(
      { id: this._domId(), className: this._classNames().join(' ') },
      this._renderCell()
    )
  }

  _renderCell() {
    return [h.div('.cell-contents', this._renderCellContents())]
  }

  _renderCellContents() {
    return [this._renderVal(), this._renderActions()]
  }
