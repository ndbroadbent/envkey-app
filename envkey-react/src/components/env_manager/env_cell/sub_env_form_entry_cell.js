...
export default class SubEnvFormEntryCell extends Autocompletable(
  FormEntryCell
) {
  constructor(props) {
    super(props)
  ...
  _classNames() {
    return super
      ._classNames()
      .concat([this.state.hidingAutocomplete ? 'hiding-autocomplete' : ''])
  }
      ...
      filterFn = val =>
        ...
        (val.trim().toLowerCase() != searchStr.trim().toLowerCase() &&
          val
            .trim()
            .toLowerCase()
            .indexOf(searchStr.trim().toLowerCase()) == 0)
