...
const CommitPromptable = Editable =>
  class extends Editable {
    ...
    _renderCommitPrompt() {
      const className =
        'commit-prompt ' + (this.state.showCommitPrompt ? 'show' : '')

      return h.div({ className }, [
        h.div('.col-left', [h.span([h.em('esc'), ' to cancel'])]),
        h.div('.col-right', [h.span([h.em('enter'), ' to commit'])]),
      ])
    }
  }
