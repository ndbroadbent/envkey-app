...
export default class SubEnvsList extends React.Component {
  ...
  _onDeleteConfirmFn(id) {
    return e =>
      this.props.removeSubEnv({ environment: this.props.environment, id })
  }
  ...
  _renderContent() {
    return [this._renderHeader(), this._renderList()]
  }
        ...
        : R.partial(h, [
            Link,
            {
              ...
              to: this.props.location.pathname.replace(
                new RegExp(`/${this.props.params.sub}/.*$`),
                `/${this.props.params.sub}/add`
              ),
            },
          ])
