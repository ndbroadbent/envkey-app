...
export default class LabelRow extends React.Component {
  ...
  _locked(environment) {
    return (
      this.props.parent.role == 'development' && environment == 'production'
    )
  }

  _hasSubEnvs(environment) {
    return !R.isEmpty(
      R.pathOr({}, [environment, '@@__sub__'], this.props.envsWithMeta)
    )
  }
    ...
    return h.div(
      '.label-cell',
      {
        ...
        className: `env-${environment} ${
          this._locked(environment) ? 'locked' : ''
        } ${environment == this.state.menuOpen ? 'menu-open' : ''}`,
      },
      [
        this._renderSubEnvsAction(environment),
        h.label([
          this._locked(environment)
            ? h.img('.img-locked', { src: imagePath('padlock.svg') })
            : '',
    ...
    if (!(parent.role == 'development' && !this._hasSubEnvs(environment))) {
      if (isSubEnvsLabel) {
        return h(
          Link,
          {
            className: 'close-subenvs',
            to: location.pathname.replace(new RegExp(`/${params.sub}/.*$`), ''),
          },
          [h.i('←')]
        )
      } else {
        return h(
          Link,
          {
            className: 'open-subenvs',
            to: location.pathname + `/${environment}/first`,
          },
  ...
  _renderActionsToggle(environment) {
    if (
      !this._locked(environment) &&
      !(this.props.isSubEnvsLabel && !this.props.subEnv)
    ) {
      return h.span(
        '.toggle-menu',
        { onClick: this._onToggleMenuFn(environment) },
        [h.i(['...'])]
      )
    }
  }
  ...
  _renderActionsMenu(environment) {
    return h.ul('.actions-menu', [
      ...
      h.li(
        {
          onClick: e =>
            this.setState({ importOpen: environment, menuOpen: null }),
        },
        [h.span('Import')]
      ),
  ...
  _renderExportAction(environment) {
    if (appRoleIsAdmin(this.props.parent.role)) {
      return h.li(
        {
          onClick: e =>
            this.setState({ exportOpen: environment, menuOpen: null }),
        },
        [h.span('Export')]
      )
    }
