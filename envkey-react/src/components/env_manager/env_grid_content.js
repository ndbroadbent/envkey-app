...
export default class EnvGridContent extends EditableCellsParent(
  React.Component
) {
  constructor(props) {
    super(props)
  ...
  _renderSocketAddingEntries() {
    const socketAddingEntries =
      this.props.socketAddingEntry[this.props.subEnvId || '@@__base__'] || []

    return h.div(
      '.socket-adding-entries-block',
      socketAddingEntries.map(({ firstName, lastName }) => {
        return h.div([
  ...
  _renderEntryRows() {
    return h.div('.vars-block', [
      h.div(
        '.entry-rows',
        allEntries(this.props.envsWithMeta).map(::this._renderEntryRow)
      ),
    ])
  }
  ...
  _renderEntryRow(entryKey) {
    ...
    return h(toClass(EntryRow), {
      ...
      socketUserEditingEntry: R.path(
        [entryKey, this.props.subEnvId || '@@__base__'],
        this.props.socketEditingEntry
      ),
      socketUserRemovingEntry: R.path(
        [entryKey, this.props.subEnvId || '@@__base__'],
        this.props.socketRemovingEntry
      ),
