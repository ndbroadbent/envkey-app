...
const EditableCellsParent = CellsParent =>
  class extends CellsParent {
    componentDidMount() {
    ...
    _onBodyClick(e) {
      ...
      if (this.state.editing.entryKey) {
        const noValNoChange =
            !this.state.editing.inputVal && !this.state.editing.defaultInputVal,
          noChange =
            this.state.editing.inputVal == this.state.editing.defaultInputVal,
        ...
        if (!(noValNoChange || noChange) && !isCurrentCell) {
          alert(
            `You're still editing '${this.state.editing.entryKey.toUpperCase()}'. Use ESCAPE to cancel or ENTER to commit.`
          )
          e.preventDefault()
          e.stopPropagation()
