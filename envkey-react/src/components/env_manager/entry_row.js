...
export default class EntryRow extends React.Component {
  render() {
    ...
    const classNames = [
      ...
      { firstName: editingFirstName, lastName: editingLastName } =
        socketUserEditingEntry || {},
      { firstName: removingFirstName, lastName: removingLastName } =
        socketUserRemovingEntry || {},
      ...
      productionLocked = Boolean(
        R.path(['production', entryKey, 'locked'], envsWithMeta)
      ),
      hasProductionMetaOnly = Boolean(
        R.path(['productionMetaOnly', entryKey], envsWithMeta)
      ),
      entryEditable =
        !(
    ...
    return h.div('.row.entry-row', { className: classNames.join(' ') }, [
      h.div(
        '.socket-entry-overlay.socket-entry-edit',
        {
      ...
      h.div(
        '.socket-entry-overlay.socket-entry-remove',
        {
      ...
      h.div('.entry-col', [
        h(entryCellClass, {
          ...
          isEditing:
            editing.entryKey === entryKey &&
            editing.subEnvId == subEnvId &&
            !editing.environment,
        }),
      ]),
        ...
        environments.map((environment, i) => {
          ...
          return h.div('.val-col', { key: i }, [
            h(valCellClass, {
              ...
              socketUserEditingEntryVal: R.path(
