...
export default class EntryForm extends React.Component {
  ...
  _classNames() {
    const autocompleteOpen =
        this.props.autocompleteOpenEnvironment &&
        this.props.autocompleteOpenEntryKey === null,
      lastEnv = this.props.environments[this.props.environments.length - 1],
      autocompleteLastEnvironmentOpen =
        autocompleteOpen && lastEnv === this.props.autocompleteOpenEnvironment

    return [
      'entry-form',
      autocompleteLastEnvironmentOpen
        ? 'autocomplete-open-last-environment'
        : '',
    ]
  }
  ...
  _renderSubmit() {
    if (
      !(
        this.props.editingMultilineEnvironment &&
        !this.props.editingMultilineEntryKey
      )
    ) {
