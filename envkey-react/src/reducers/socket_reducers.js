...
export const socketIsUpdatingEnvs = (state = {}, action) => {
    ...
    switch (action.type) {
      ...
      case FETCH_OBJECT_DETAILS_FAILED:
        if (action.meta && action.meta.socketUpdate) {
          return R.pipe(
            R.dissocPath([
              action.meta.socketActorId,
              action.meta.socketEnvUpdateId,
            ]),
            R.reject(R.isEmpty)
          )(state)
        } else {
  ...
  pendingLocalSocketEnvsStatus = (state = {}, action) => {
    ...
    switch (action.type) {
      case CREATE_ENTRY:
        return R.assocPath(
          [action.meta.envUpdateId, 'addingEntry'],
          action.payload.subEnvId || '@@__base__',
          state
        )
