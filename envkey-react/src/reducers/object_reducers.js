...
const getFetchCurrentUserReducer = objectTypePlural => (state, { payload }) => {
  ...
  getFetchCurrentUserUpdatesReducer = objectTypePlural => (state, action) => {
    ...
    if (action.meta && action.meta.noMinUpdatedAt) {
      ...
      return objects && objects.length
        ? { ...state, ...indexById(objects) }
        : state
    }
  },
  getUpdateEnvReducer = objectTypePlural => (
    state,
    {
  ...
  getRemoveAssocReducer = objectTypePlural => (
    state,
    { meta: { parentType, assocType, targetId, isManyToMany } }
  ) => {
    const type = isManyToMany
      ? pluralize(camelize([parentType, assocType].join('_')))
      : pluralize(assocType)
  ...
  getAddAssocReducer = objectTypePlural => (
    state,
    { meta: { parentType, assocType, isManyToMany }, payload }
  ) => {
    const type = isManyToMany
      ? pluralize(camelize([parentType, assocType].join('_')))
      : pluralize(assocType)
    if (type == objectTypePlural) {
      ...
      objectTypePlural == 'apps' &&
      parentType == 'app' &&
      payload.keyablesUpdatedAt
    ) {
      return R.assocPath(
        [payload.appId, 'keyablesUpdatedAt'],
        payload.keyablesUpdatedAt,
        state
      )
    }
  ...
  getCreateObjectReducer = objectTypePlural => (
    state,
  ...
  getUpdateObjectReducer = objectTypePlural => (
    state,
    { meta: { objectType }, payload }
  ) => {
    if (pluralize(objectType) == objectTypePlural) {
  ...
  getGenerateKeyReducer = objectTypePlural => (
    state,
  ...
  getRevokeKeyReducer = objectTypePlural => (
    state,
    { meta: { assocType }, payload }
  ) => {
    if (pluralize(assocType) == objectTypePlural) {
  ...
  getDecryptEnvsReducer = objectTypePlural => (
    state,
    { meta: { objectType }, payload }
  ) => {
    if (pluralize(objectType) == objectTypePlural) {
  ...
  getFetchObjectReducer = objectTypePlural => (
    state,
    { meta: { objectType }, payload }
  ) => {
    if (pluralize(objectType) == objectTypePlural) {
  ...
  getFetchObjectAssociationsReducer = objectTypePlural => (
    state,
    { meta: { objectType }, payload }
  ) => {
    if (pluralize(objectType) != objectTypePlural) {
  ...
  objectReducers[objectTypePlural] = (state = {}, action) => {
    ...
    switch (action.type) {
      ...
      case FETCH_CURRENT_USER_UPDATES_API_SUCCESS:
        return getFetchCurrentUserUpdatesReducer(objectTypePlural)(
          state,
          action
        )
      ...
      case FETCH_OBJECT_DETAILS_API_SUCCESS:
        return getFetchObjectAssociationsReducer(objectTypePlural)(
          state,
          action
        )
  ...
  switch (action.type) {
    ...
    case REMOVE_OBJECT_SUCCESS:
      if (
        ...
        action.type == REMOVE_OBJECT_SUCCESS &&
        action.meta.objectType == 'app' &&
        action.meta.isOnboardAction
      ) {
        return null
