...
import {
  AUTH_KEYS,
  LOCAL_PERSISTENCE_AUTH_KEYS,
  SESSION_PERSISTENCE_AUTH_KEYS,
} from 'constants'
...
const logger = createLogger(loggerOpts),
  ...
  sessionAdapter = isElectron()
    ? electronStorageAdapter
    : localStorageAdapter(window.localStorage),
  privkeyAdapter = isElectron()
    ? electronStorageAdapter
    : sessionStorageAdapter(window.sessionStorage),
  sessionPersistence = compose(filter(LOCAL_PERSISTENCE_AUTH_KEYS))(
    sessionAdapter
  ),
  privkeyPersistence = compose(filter(SESSION_PERSISTENCE_AUTH_KEYS))(
    privkeyAdapter
  ),
