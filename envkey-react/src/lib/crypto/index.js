...
let currentProxy = 0
const workerPath = `${process.env.ASSET_HOST}/openpgp.worker.min.js`,
  ...
  initProxyPool = () => {
    ...
    while (i < concurrency) {
      proxyPool.push(
        new openpgp.AsyncProxy({ path: workerPath, config: openpgp.config })
      )
      i++
    }
  },
...
export const init = () => {
  ...
  secureRandomAlphanumeric = len => {
    return bs58
      .encode(openpgp.crypto.random.getRandomBytes(Math.ceil(len * 0.75)))
      .slice(0, len)
  },
  ...
  generateKeys = ({ email, passphrase }, worker = false) => {
    ...
    return worker
      ? proxy().delegate('generateKey', opts)
      : openpgp.generateKey(opts)
  },
  getPubkeyFingerprint = pubkey =>
    openpgp.key.readArmored(pubkey).keys[0].primaryKey.fingerprint,
  ...
  verifyPublicKeySignature = ({
    signedKey: signedKeyArmored,
    signerKey: signerKeyArmored,
  }) => {
