...
export const UPDATE_ENVS = 'UPDATE_ENVS',
...
const ORG_MESSAGE_TYPES = {
  ...
  pusherClient = (auth, orgSlug) => {
    return new Pusher(process.env.PUSHER_APP_KEY, {
      ...
      }/${API_VERSION}/pusher/subscribe.json`,
      auth: {
  ...
  bindChannel = (messageTypes, channel) => {
    ...
    messageTypes.clientEvents.forEach(type => {
      channel.bind('client-' + type, data =>
        store.dispatch(socketAction(type, data))
      )
    })
  },
  ...
  bindPresenceChannel = (eventType, actionCreator, channel) => {
    channel.bind(eventType, ({ id: userId }) =>
      store.dispatch(actionCreator({ userId }))
    )
  },
...
export const ensureSocket = (auth, orgSlug) => {
  ...
  subscribeOrgChannels = (org, currentUser) => {
    ...
    orgPresenceChannel = socket.subscribe(
      org.broadcastChannel.replace(/^private-/, 'presence-')
    )
  ...
  subscribeObjectChannel = object => {
    ...
    objectPresenceChannel = socket.subscribe(
      object.broadcastChannel.replace(/^private-/, 'presence-')
    )
