...
import { AppForm, ServerForm, UserForm, LocalKeyForm } from 'components/forms'
...
export default function({ parentType, assocType, parent, state }) {
  switch ([parentType, assocType].join('-')) {
    case 'app-user':
      const userGroups = getUserGroupsByRoleForApp(parent.id, state),
        ...
        connectedUsersWithoutRelations = dissocRelations(
          getUsersForApp(parent.id, state)
        ),
      ...
      return {
        ...
        addExistingTextFn: ({ firstName, lastName }) =>
          [firstName, lastName].join(' '),
        addExistingLabelFn: ({ firstName, lastName }) =>
          h.span([h.span([firstName, ' ']), h.strong(lastName)]),
        ...
        columns: [
          {
            ...
            isAddingAssoc:
              getIsAddingAssoc(
                { assocType, parentId: parent.id, role: 'admin' },
                state
              ) ||
              getIsAddingAssoc(
                { assocType, parentId: parent.id, role: 'org_admin' },
                state
              ),
            isCreating:
              getIsCreating(
          ...
          },
          {
            ...
            keyLabel: 'development',
            isAddingAssoc: getIsAddingAssoc(
              { assocType, parentId: parent.id, role: 'production' },
              state
            ),
          ...
          },
          {
            ...
            keyLabel: 'development',
            isAddingAssoc: getIsAddingAssoc(
              { assocType, parentId: parent.id, role: 'development' },
              state
            ),
            isCreating: getIsCreating(
              { assocType, parentId: parent.id, role: 'development' },
    ...
    case 'app-server':
      ...
      return {
        ...
        addLabel: ({ title }) => [
          h.span(`Add another ${title.toLowerCase()} server key`),
          h.span('.img', [h.img({ src: imagePath('circle-plus-white.svg') })]),
        ],
        removeLabel: [h.span('Cancel'), h.span('.img', [h.span('x')])],
        inlineAddForm: true,
        canAddFn: R.anyPass([
          R.pipe(
            R.path(['config', 'groups']),
            R.values,
            R.head,
            R.length,
            R.lt(1)
          ...
          R.pipe(
            R.path(['config', 'groups']),
            R.values,
            R.any(R.any(R.prop('keyGeneratedAt')))
          ),
        ]),
        showSectionFn: (subEnvId, props) => {
          return Boolean(
            !subEnvId ||
              subEnvId == 'null' ||
              !props.parent.envsWithMeta ||
              findSubEnv(subEnvId, props.parent.envsWithMeta)
          )
        },
        sectionTitleFn: (subEnvId, props) => {
          ...
          return subEnvId == 'null'
            ...
            : [
                ...
                h.span(
                  `${findSubEnv(subEnvId, parent.envsWithMeta)['@@__name__']}`
                ),
              ]
        },
        ...
        columns: [
          ...
          },
          {
            ...
            keyLabel: 'staging',
            permissionCopyLines: [
              h.span(['Connect to the ', h.strong('staging'), ' environment.']),
            ],
            isAddingAssoc: getIsAddingAssoc(
              { assocType, parentId: parent.id, role: 'staging' },
              state
            ),
            isCreating: getIsCreating(
              { assocType, parentId: parent.id, role: 'staging' },
    ...
    case 'app-localKey':
      return {
        ...
        addLabel: [
          h.span('Add another local development key'),
          h.span('.img', [h.img({ src: imagePath('circle-plus-white.svg') })]),
        ],
        removeLabel: [h.span('Cancel'), h.span('.img', [h.span('x')])],
        inlineAddForm: true,
        ...
        columns: [
          {
            ...
            role: 'development',
            groups: {
              development: getCurrentUserLocalKeysForApp(parent.id, state),
            },
            keyLabel: 'development',
    ...
    case 'user-app':
      const appGroups = getAppGroupsForUser(parent.id, state),
        ...
        orgAdminApps = R.pipe(
          R.pick(orgAdminRoles),
          R.values,
          R.flatten
        )(appGroups),
        appAdminRoles = orgAdminRoles.concat(['admin']),
        connectedAppsWithoutRelations = dissocRelations(
          getAppsForUser(parent.id, state)
        ),
      ...
      return {
        ...
        parentNameFn: ({ firstName, lastName }) =>
          [firstName, lastName].join(' '),
        columns,
      }
