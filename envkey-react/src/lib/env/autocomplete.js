...
const getEnvsFilter = (
  { entryKey, envsWithMeta, environment },
  searchStr
) => envOpt => {
  ...
  while (inherited.inherits) {
    ...
    inherited = inheritedShallow({
      entryKey,
      envsWithMeta,
      inherits: inherited.inherits,
    })
  }
...
export const optIndex = (searchStr, opts) =>
  R.findIndex(({ val }) => val === searchStr)(opts)

export const getAutocompleteOpts = (
  { entryKey, inherits, val, envsWithMeta, environments, environment },
  searchStr
) => {
  const searchStrIsEnv = environments.includes(searchStr),
    baseOpts =
      searchStr && !searchStrIsEnv
        ? []
        : [
            {
              val: null,
    ...
    envOpts = environments
      .filter(getEnvsFilter({ entryKey, envsWithMeta, environment }, searchStr))
      .map(envOpt => ({
        val: envOpt,
        label: envOpt,
  ...
  return hasMatch
    ? opts
    : opts.filter(({ val }) => val && val.indexOf(searchStr) == 0)
}
