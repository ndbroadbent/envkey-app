...
const isEntry = (v, k) => k.indexOf('@@__') != 0,
  ...
  productionMetaValToLockedProductionVal = ({
    inherits,
    hasVal,
    isUndefined,
    isEmpty,
  ...
  prodValToProdMetaOnlyValFn = (envsWithMeta, keyPath = []) => (
    entryVal,
    entryKey
  ) => {
    ...
    if (!entryVal || entryVal.locked) {
      return R.path(
        ['productionMetaOnly'].concat(keyPath).concat(entryKey),
        envsWithMeta
      )
    }
  ...
  withProductionMetaOnly = envsWithMeta => {
    ...
    return {
      ...envsWithMeta,
      productionMetaOnly: R.mapObjIndexed((entryVal, entryKey) => {
        if (entryKey == '@@__sub__') {
          const subEnvs = entryVal
          return R.mapObjIndexed((subEnv, subEnvId) => {
            const keyPath = ['@@__sub__', subEnvId],
              prodValToProdMetaOnlyVal = prodValToProdMetaOnlyValFn(
                envsWithMeta,
                keyPath
              )
    ...
    if (prodVal || key.indexOf('@@__') == 0) {
      ...
      const lockedVal = prodMetaVal
        ...
        : {
            ...
            inherits: null,
            locked: true,
            hasVal: !envsWithMeta.productionMetaOnly,
          }
  ...
  withLockedProduction = envsWithMeta => {
    const keys = allKeys(envsWithMeta),
      prodVals = R.map(key => {
        if (
          key == '@@__sub__' &&
          R.path(['productionMetaOnly', '@@__sub__'], envsWithMeta)
        ) {
          return {
            '@@__sub__': R.mapObjIndexed(
              (subEnv, subEnvId) =>
                R.mergeAll(
                  R.map(
                    prodKeyWithLockedProductionFn(envsWithMeta, [
                      '@@__sub__',
                      subEnvId,
                    ]),
                    R.keys(subEnv)
...
export const normalizeEnvsWithMeta = R.pipe(
    withMissingEntries,
  ...
  createEntry = subEnvUpdatable(({ envsWithMeta, entryKey, vals }) => {
    return R.mapObjIndexed((env, name) => {
  ...
  updateEntryVal = subEnvUpdatable(
    ({ envsWithMeta, entryKey, environment, update }) => {
      const path = [environment, entryKey],
      ...
      const inheriting = inheritingEnvironments({
        environment,
        entryKey,
        envsWithMeta,
      })
      ...
      if (inheriting.length) {
        for (let inheritingEnvironment of inheriting) {
          if (update.val) {
            updated = R.assocPath(
              [inheritingEnvironment, entryKey, 'inheritedVal'],
              update.val,
              updated
            )
          } else {
            updated = R.dissocPath(
              [inheritingEnvironment, entryKey, 'inheritedVal'],
              updated
            )
          }
  ...
  envUpdateConflicts = (
    ...
    const updateDiffs = diff(preUpdateEnvsWithMeta, postUpdateEnvsWithMeta),
      preUpdateWithPending = envActionsPending.reduce(
        transformEnv,
        preUpdateEnvsWithMeta
      ),
      postUpdateWithPending = envActionsPending.reduce(
        transformEnv,
        postUpdateEnvsWithMeta
      ),
      preUpdatePendingDiffs = diff(preUpdateEnvsWithMeta, preUpdateWithPending),
      allEntriesSet = new Set(
        R.concat(
      ...
      commonPaths = R.innerJoin(
        (p1, p2) =>
          R.equals(p1, p2) || R.startsWith(p1, p2) || R.startsWith(p2, p1),
