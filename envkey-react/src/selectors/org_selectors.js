...
export const getCurrentOrgSlug = db.path('currentOrgSlug'),
  ...
  getOrgs = db.orgs.list({
    sortBy: o =>
      [
        o.role == 'org_owner' ? 0 : 1,
        Date.parse(o.accessStatus.timestamp),
      ].join('-'),
  }),
  getOrgsLoaded = db.path('orgsLoaded'),
  getActiveOrgs = db.orgs.where(
    { isActive: true },
    {
      sortBy: o =>
        [
  ...
  getIsExceedingFreeTier = state => {
    ...
    return (
      getApps(state).length > maxApps || getActiveUsers(state).length > maxUsers
    ) // ||
