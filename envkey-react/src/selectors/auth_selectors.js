...
import { getCurrentOrg } from './org_selectors'
import {
...
export const getAppLoaded = db.path('appLoaded'),
  ...
  getCurrentUserEnvironmentsAccessibleForApp = R.curry((appId, state) => {
    ...
    return getEnvironmentsAccessibleForAppUser(
      { userId: auth.id, appId },
      state
    )
  }),
  getCurrentUserEnvironmentsAssignableForApp = R.curry((appId, state) => {
    ...
    return getEnvironmentsAssignableForAppUser(
      { userId: auth.id, appId },
      state
    )
  }),
  getCurrentUserEnvironmentsAssignableToAppUser = R.curry(
    ({ appId, userId, role }, state) => {
      const assignable = getCurrentUserEnvironmentsAssignableForApp(
          appId,
          state
        ),
        accessible = getEnvironmentsAccessibleForAppUser(
          { appId, userId, role },
          state
        )
  ...
  getCurrentUserEnvironmentAssignableToServer = R.curry(
    ({ appId, serverId }, state) => {
      const currentUserEnvironmentsAssignable = getCurrentUserEnvironmentsAssignableForApp(
          appId,
          state
        ),
  ...
  getEnvironmentsAccessible = R.curry((appId, state) => {
    const environments = getCurrentAppUserForApp(appId, state)
      .environmentsAccessible
  ...
  getEnvironmentsAccessibleWithSubEnvs = R.curry((appId, state) => {
    return getEnvironmentsAccessible(appId, state).concat(
      getSubEnvs(appId, state)
    )
  }),
