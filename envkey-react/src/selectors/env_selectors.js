...
export const getEnvActionsPendingByEnvUpdateId = R.curry(
    (parentId, envUpdateId, state) => {
  ...
  getLastAddedEntry = (parentId, state) =>
    db.path('lastAddedEntry', parentId)(state),
  ...
  getEnvsWithMetaWithPending = defaultMemoize(
    R.curry((parentType, parentId, state) => {
  ...
  getEnvsWithMetaWithPendingWithImports = defaultMemoize(
    R.curry((parentType, parentId, state) => {
      const envsWithMeta = getEnvsWithMetaWithPending(
          parentType,
          parentId,
          state
        ),
  ...
  getRawEnvWithPendingForApp = R.curry((opts, state) => {
    const { appId, environment, subEnvId } = opts,
  ...
  getEnvironmentsAccessibleForAppUser = R.curry(
  ...
  getEnvironmentsAssignableForAppUser = R.curry(
    ({ appId, userId, role }, state) => {
      let environments
