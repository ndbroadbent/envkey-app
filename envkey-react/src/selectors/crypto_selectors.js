...
export const getIsGeneratingUserKey = db.path('isGeneratingUserKey'),
  ...
  getIsDecryptingAll = R.anyPass([
    getIsDecryptingAllForeground,
    getIsDecryptingAllBackground,
  ]),
  getIsDecrypting = R.anyPass([
    getIsDecryptingPrivkey,
    getIsDecryptingAllForeground,
  ]),
  ...
  getTrustedPubkeyChain = state => {
    ...
    while (true) {
      if (checked[currentId])
        throw new Error("Trusted pubkey chain couldn't find org creator root.")
      ...
      if (!trustedInviter)
        throw new Error('Trusted user not found. Chain broken.')

      trustChain[currentId] = R.pick(
        ['invitePubkey', 'pubkey', 'invitedById'],
        trustedInviter
      )
