...
class StripeCardForm extends React.Component {
  ...
  _submitCopy() {
    return { upgrade_subscription: 'Upgrade', update_payment: 'Update' }[
      this._formType()
    ]
  }
  ...
  _renderHeader() {
    return h.header([
      h.div('.logo', [h.img({ src: imagePath('envkey-logo.svg') })]),
    ])
  }
  ...
  _userCols() {
    ...
    if (numUsersPending) {
      cols.push(
        <small>
          + {numUsersPending} pending invitation
          {numUsersPending == 1 ? '' : 's'}
        </small>
      )
    }
  ...
  _chargeCols() {
    ...
    if (plan.pricingVersion >= 4) {
      ...
      cols = [
        <strong>${parseInt((plan.amount * numUsersActive) / 100)}.00</strong>,
      ]
    }
    ...
    if (numUsersPending && plan.pricingVersion < 4) {
      cols.push(
        <small>
          + ${parseInt((plan.amount * numUsersPending) / 100)}.00 pending
        </small>
      )
    }
  ...
  _renderInfo() {
    if (this._formType() == 'upgrade_subscription') {
      ...
      return h.section('.plan-info', [
        ...
        h.h3(plan.name),
        h.div('.billing-info', [
          BillingColumns({ columns: [[this._renderPricing()]] }),
  ...
  _renderPricing() {
    const { plan } = this._formData()
    return plan.pricingVersion >= 4
      ? this._renderPricingPostV4()
      : this._renderPricingPreV4()
  }
  ...
  _renderPricingPostV4() {
    const { plan } = this._formData()
    return [
      `$${parseInt(plan.amount / 100)}.00 per month`,
      [
  ...
  _renderError() {
    ...
    if (error) {
      return h.section('.error', [
        h.div('.top', [h.span('.error-message', error)]),
        h.div('.bottom', [h.span('Please re-enter your card details.')]),
      ])
    }
  }
}
...
const Root = h(StripeProvider, { apiKey: process.env.STRIPE_PUBLISHABLE_KEY }, [
  h(Elements, [h(injectStripe(StripeCardForm))]),
])
